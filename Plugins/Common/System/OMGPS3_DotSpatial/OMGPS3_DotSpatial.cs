...
namespace OMGPS3_DotSpatial
{
    public sealed class OMGPS3_DotSpatial : HighLevelCode, IDataSource
    {
        ...
        public OMGPS3_DotSpatial()
            : base("OMGPS3_DotSpatial", OM.Host.getPluginImage("OMGPS3_DotSpatial", "Icon-GPS"), 0.1f, "GPS Data", "GPS Data", "OM DevTeam/Borte", "")
        {
        }
        ...
        private void Devices_DeviceDetectionCompleted(object sender, EventArgs e)
        {
            if (_GPSDevice != null)
            {
                ...
                {
                    _GPSStatusNotification_SetData(text: String.Format("GPS connection failed", _GPSDevice));
                }
            }
            else
            {
        ...
        private void UpdateGPSSatView()
        {
            OM.Host.ForEachScreen((int screen) =>
            {
                ...
                foreach (Satellite sat in _NmeaInterpreter.Satellites)
                {
                    ...
                    int satX = (int)(center.X + h * Math.Sin(sat.Azimuth.ToRadians().Value));
                    int satY = (int)(center.Y - h * Math.Cos(sat.Azimuth.ToRadians().Value));
        ...
        private void DataSources_Register()
        {
            ...
            OM.Host.DataHandler.AddDataSource(new OpenMobile.Data.DataSource(this.pluginName, "GPS", "Bearing", "Angle", OpenMobile.Data.DataSource.DataTypes.raw, "GPS bearing angle as an int"), 0);
        }
        ...
        private void _NmeaInterpreter_FixLost(object sender, EventArgs e)
        {
            ...
            _GPSStatusNotification_SetData(text: "GPS Fix lost!", iconStatusBar: _IconGPSNotFixed.image, updatePanelText: false);
        }
        ...
        {
            _GPSStatusNotification_SetData(header: "GPS connected", text: "Waiting for data...");
        }
