...
struct graphics_priv {
	int button_timeout;
	struct point p;
	int window_start_pos_x;
	int window_start_pos_y;
	int width;
	int height;
	int library_init;
	int visible;
...
#else
/*  prototypes */
void APIENTRY opengl_external_tess_begin_cb(GLenum which);
void APIENTRY opengl_external_tess_end_cb();
void APIENTRY opengl_external_tess_error_cb(GLenum errorCode);
void APIENTRY opengl_external_tess_vertex_cb(const GLvoid * data);
void APIENTRY opengl_external_tess_vertex_cb2(const GLvoid * data);
void APIENTRY opengl_external_tess_combine_cb(GLdouble c[3], void *d[4], GLfloat w[4], void **out);
const char *opengl_external_get_primitive_type(GLenum type);
#endif

static struct graphics_priv *graphics_opengl_external_new_helper(struct
							graphics_methods
							*meth);
...
// external callbacks
static void (PINVOKEENTRY *external_callback_debug_out)(char*, int) ;
static int (PINVOKEENTRY *external_callback_event_add)(int ms) ;
static void (PINVOKEENTRY *external_callback_event_remove)(int timer_id) ;
 
static void external_debug(char* debugString)
{
	if(external_callback_debug_out != NULL)
		external_callback_debug_out(debugString, 0);
}
...
#else
const char *
opengl_external_get_primitive_type(GLenum type)
{
	char *ret = "";
...
void APIENTRY
opengl_external_tess_begin_cb(GLenum prim)
{
	glBegin(prim);

	dbg(1, "glBegin( %s );\n", opengl_external_get_primitive_type(prim));
}



void APIENTRY
opengl_external_tess_end_cb()
{
	glEnd();
...
void APIENTRY
opengl_external_tess_vertex_cb(const GLvoid * data)
{
...
#else
void APIENTRY
opengl_external_tess_combine_cb(GLdouble c[3], void *d[4], GLfloat w[4], void **out)
{
...
void APIENTRY
opengl_external_tess_error_cb(GLenum err)
{
	char buffer[50];
	sprintf(buffer, "opengl_external_tess_error_cb: %d", err);
	external_debug(buffer);
	     ...
	     struct point *p, int count)
{
	...
	gluTessCallback(tess, GLU_TESS_BEGIN,		(void (APIENTRY *)(void)) opengl_external_tess_begin_cb);
	gluTessCallback(tess, GLU_TESS_END,			(void (APIENTRY *)(void)) opengl_external_tess_end_cb);
	gluTessCallback(tess, GLU_TESS_ERROR,		(void (APIENTRY *)(void))opengl_external_tess_error_cb);
	gluTessCallback(tess, GLU_TESS_VERTEX,		(void (APIENTRY *)(void)) opengl_external_tess_vertex_cb);
	gluTessCallback(tess, GLU_TESS_COMBINE,		(void (APIENTRY *)(void)) opengl_external_tess_combine_cb);
...
static void
opengl_external_handle_mouse_queue()
{
#if USE_OPENGLES
#else
	// For some reason this was not being "unlocked" in some circumstances.
	// This should only ever be called from a single thread so this is really unnecessary
	static locked = 0;
	if (!locked) {
		locked = 1;
	} else {
...
redraw_screen(struct graphics_priv *gr)
{
	...
	glCallList(gr->DLid);
	char buffer[200];
	sprintf(buffer, "glCallList %d", gr->DLid);
	external_debug(buffer);
	...
	while (gr->overlay_enabled && overlay) {
		if (overlay->overlay_enabled) {
			...
			sprintf(buffer, "glCallList (overlay) %d", overlay->DLid);
			external_debug(buffer);
...
static char* get_draw_mode_string(enum draw_mode_num mode)
{
	switch(mode)
	{
		case draw_mode_begin:
			return "begin";
		case draw_mode_begin_clear:
			return "begin_clear";
		case draw_mode_end:
			return "end";
		case draw_mode_cursor:
			return "cursor";
		case draw_mode_end_lazy:
			return "end_lazy";
	}
	return "invalid mode";
}
...
draw_mode(struct graphics_priv *gr, enum draw_mode_num mode)
{
	...
	strcat(buffer, "mode: ");
	strcat(buffer, get_draw_mode_string(mode));
	...
	if (gr->parent) {	//overlay
		...
		if (mode == draw_mode_begin) {
			...
			if(gr->callListNestLevel == 0)
			{
				glNewList(gr->DLid, GL_COMPILE);
				sprintf(buffer, "Creating new display list (Parent) listId: %d", gr->DLid);
				external_debug(buffer);
			}
			gr->callListNestLevel++;
		}
		...
		if (mode == draw_mode_end || mode == draw_mode_end_lazy) {
			...
			if(gr->callListNestLevel == 1)
			{
				glEndList();
				external_debug("EndDraw (Parent)");
			}
			if(gr->callListNestLevel > 0)
				gr->callListNestLevel--;
		}
		...
		if (mode == draw_mode_begin) {
			...
			{
				glNewList(gr->DLid, GL_COMPILE);
				sprintf(buffer, "Creating new display list (NotParent) listId: %d", gr->DLid);
				external_debug(buffer);
			}
			gr->callListNestLevel++;
#endif
		...
		if (mode == draw_mode_end) {
			...
			{
				glEndList();
				external_debug("End Display List");
			
				external_debug("DrawMode() -> RedrawScreen");
	...
	sprintf(buffer, "exiting draw_mode. fuck is %d", fuck);
	external_debug(buffer);
...
static int
graphics_opengl_external_fullscreen(struct window *w, int on)
{
	return 1;
}

static void
graphics_opengl_external_disable_suspend(struct window *w)
{
}
...
get_data(struct graphics_priv *this, char *type)
{
	...
	if (strcmp(type, "window") == 0) {
		...
		win->priv = this;
		win->fullscreen = graphics_opengl_external_fullscreen;
		win->disable_suspend = graphics_opengl_external_disable_suspend;
		return win;
	} else {
#if USE_OPENGLES
		return NULL;
...
graphics_opengl_external_new_helper(struct graphics_methods *meth)
{
...
{
	int w2, h2;
	struct graphics_priv *this = graphics_opengl_external_new_helper(meth);
	this->p = *p;
	this->width = w;
	this->height = h;
	this->parent = gr;
...
#if USE_OPENGLES
#else
	external_debug("glGenList (overlay)");
...
{
	int queueIndex = mouse_event_queue_end_idx % mouse_event_queue_size;

	mouse_queue[queueIndex].button = button;
	mouse_queue[queueIndex].state = state;
#ifdef MIRRORED_VIEW
	mouse_queue[queueIndex].x = graphics_priv_root->width - x;
#else
	mouse_queue[queueIndex].x = x;
#endif
	mouse_queue[queueIndex].y = y;
...
static struct graphics_priv *
graphics_opengl_external_new(struct navit *nav, struct graphics_methods *meth,
		    struct attr **attrs, struct callback_list *cbl)
{
	struct attr *attr;

	
	if (!event_request_system("opengl_external", "opengl_external"))
			return NULL;
	//if (!event_request_system("glib", "graphics_opengl_external_new"))
	//	return NULL;
	...
	/*event_add_timeout(
		...
		1,
		callback_new_0(callback_cast(handle_mouse_queue)));*/

	//*this will only refresh screen in map(overlay enabled) mode */
	//event_opengl_external_add_timeout(G_PRIORITY_DEFAULT + 1000, redraw_filter, this);
	/*
	event_add_timeout(
		G_PRIORITY_DEFAULT + 1000, 
		1,
		callback_new_1(callback_cast(redraw_filter), this));
*/
...
static void
event_opengl_external_main_loop_run(void)
{
	dbg(0, "enter\n");
}

static void
event_opengl_external_main_loop_quit(void)
{
	dbg(0, "enter\n");
}

static struct event_watch *
event_opengl_external_add_watch(void *h, enum event_watch_cond cond,
		       struct callback *cb)
{
	dbg(0, "enter\n");
	return NULL;
...
static void
event_opengl_external_remove_watch(struct event_watch *ev)
{
	dbg(0, "enter\n");
}
...
event_opengl_external_add_timeout(int timeout, int multi, struct callback *cb)
{
	...
	int eventId = 99;
	if(external_callback_event_add)
		eventId = external_callback_event_add(timeout);
    t->timer_id = eventId;
...
static void
event_opengl_external_remove_timeout(struct event_timeout *to)
{
    if (to)
    {
        GList *l;
        ...
        while (l)
        {
            ...
            if (t == to)
            {
				if(external_callback_event_remove)
					external_callback_event_remove(to->timer_id);
...
event_opengl_external_add_idle(int priority, struct callback *cb)
{
	return (struct event_idle *)event_opengl_external_add_timeout(1, 1, cb);
}

static void
event_opengl_external_remove_idle(struct event_idle *ev)
{
	event_opengl_external_remove_timeout((struct event_timeout *)ev);
	dbg(0, "enter\n");
}

static void
event_opengl_external_call_callback(struct callback_list *cb)
{
	dbg(0, "enter\n");
}

static struct event_methods event_opengl_external_methods = {
	event_opengl_external_main_loop_run,
	event_opengl_external_main_loop_quit,
	event_opengl_external_add_watch,
	event_opengl_external_remove_watch,
	event_opengl_external_add_timeout,
	event_opengl_external_remove_timeout,
	event_opengl_external_add_idle,
	event_opengl_external_remove_idle,
	event_opengl_external_call_callback,
};

static struct event_priv *
event_opengl_external_new(struct event_methods *meth)
{
	*meth = event_opengl_external_methods;
	return NULL;
}
...
plugin_init(void)
{
	plugin_register_graphics_type("opengl_external", graphics_opengl_external_new);
	 plugin_register_event_type("opengl_external", event_opengl_external_new);
}

void PINVOKEENTRY external_opengl_initialize(int x, int y, int width, int height)
{
	graphics_priv_root->width = width;
	graphics_priv_root->height = height;
	graphics_priv_root->window_start_pos_y = y;
	graphics_priv_root->window_start_pos_x = x;
	graphics_priv_root->DLid = glGenLists(1);
...
void PINVOKEENTRY external_display()
{	
	navit_draw(graphics_priv_root->nav);
}

void PINVOKEENTRY external_display_displaylist()
{
    ...
    //    l = g_list_next(l);
    //}
	struct graphics_priv *_this = graphics_priv_root;

	glMatrixMode (GL_PROJECTION);
	
	glPushMatrix();
	glLoadIdentity ();
	gluOrtho2D (-1 * _this->window_start_pos_x, _this->width, _this->height, -1 * _this->window_start_pos_y);
	...
	//navit_draw_displaylist(graphics_priv_root->nav);
}

void PINVOKEENTRY external_input_mouse_move(int x, int y)
{
	motion_notify(x, y);
}

void PINVOKEENTRY external_input_mouse_click(int button, int state, int x, int y)
{
	click_notify(button, state, x, y);
}

void PINVOKEENTRY external_input_key_press(int key_code)
{
	process_key(key_code);
}

void PINVOKEENTRY external_register_callback_debug(void  (*funcPointer)(char*, int))
{
	//return;

	external_callback_debug_out = funcPointer;
}

void PINVOKEENTRY external_register_callback_event_add(int (*funcPointer)(int))
{
	//return;

	external_callback_event_add = funcPointer;
}

void PINVOKEENTRY external_register_callback_event_remove(void (*funcPointer)(int))
{
	//return;

	external_callback_event_remove = funcPointer;
}

void PINVOKEENTRY external_event_call(int idEvent)
{
	GList *l;
    struct event_timeout *t;
    l = timers;
    ...
    {
        t = l->data;
        if (t->timer_id == idEvent || t->timer_id == 99)
        {
            struct callback *cb = t->cb;
            dbg(2, "Timer %d (multi: %d)\n", t->timer_id, t->multi);
            if (!t->multi && external_callback_event_remove != NULL)
            {
				if(external_callback_event_remove)
					external_callback_event_remove(idEvent);
                timers = g_list_remove(timers, t);				
                g_free(t);
            }
            callback_call_0(cb);
