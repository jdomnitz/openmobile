...
namespace OMGPS
{
    ...
    public class OMGPS : IHighLevel, IDataSource
    {
        ...
        Thread gpsThread;
        bool killThread;
        public Notification GPSNotification;
        public Notification DLNotification;
        public Notification ProcessingNotification;
        public Notification PostalNotification;
            ...
            while (true)
            {
                ...
                while (dataStream.Contains("\r\n"))
                {
                    ...
                    if (lineToParse.Contains("$"))
                    {
                        ...
                        OpenMobile.Threading.SafeThread.Asynchronous(() => { gpsData.Parse(lineToParse); });
                    }
    ...
    public class GPSData
    {
        ...
        public int SatCount
        {
            ...
            set
            {
                if (this._SatCount != value)
                {
                    this._SatCount = value;
                }
            }
        }
        ...
        /// <summary>
        /// Speed in system default format (kmh for metric, mph for imperial)
        /// </summary>
        public string Speed
        {
            get
        ...
        /// <summary>
        /// The raw GPS sentence as received from the GPS directly
        /// </summary>
        public string RawGPSSentence
        {
            get
            {
                return this._RawGPSSentence;
            }
            set
            {
                if (this._RawGPSSentence != value)
                {
                    this._RawGPSSentence = value;
                }
            }
        }
        private string _RawGPSSentence;
        ...
        private void GPGGAParse(string sentence)
        {
            ...
            if (data[2].ToLower() == "s")
                latitude = -latitude;
            if (latitude != 0)
                _Latitude = latitude; // Public property
            ...
            if (data[4].ToLower() == "w")
                longitude = -longitude;
            if (longitude != 0)
                _Longitude = longitude; // Public property

            //5=fix (0=invalid,1=gps fix,2=dgps fix)
            _Fixed = (!data[5].Equals("0"));

            //6=number of sats
            int.TryParse(data[6], out _SatCount);
        ...
        private void GPRMCParse(string sentence)
        {
            string[] data = sentence.Split(',');
            //0=time stamp
            //1 = validity
            ...
            if (double.TryParse(data[6], System.Globalization.NumberStyles.AllowDecimalPoint, System.Globalization.CultureInfo.InvariantCulture, out _SpeedMPH))
                _SpeedMPH *= 1.15;
            if (double.TryParse(data[6], System.Globalization.NumberStyles.AllowDecimalPoint, System.Globalization.CultureInfo.InvariantCulture, out _SpeedKPH))
                _SpeedKPH *= 1.852;

            // Push to system specific data property
            _Speed = OpenMobile.Framework.Globalization.convertSpeedToLocal(kmh, true);
