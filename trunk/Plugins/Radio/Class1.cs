...
namespace OMRadio
{
    public class OMRadio : IHighLevel
    {
                ...
                case "source":
                    {
                        ...
                        {
                            List_Source.Add(new OMListItem("No source available", "", format));
                        }
                    }
                    break;
                case "band":
                    {
                        ...
                        tunedContentBand[] lst = (tunedContentBand[])o;
                        OMListItem.subItemFormat format = new OMListItem.subItemFormat();
                        format.color = Color.FromArgb(128, Color.White);
                        format.font = new Font(FontFamily.GenericSansSerif, 21F);
                        if (lst != null)
                        {
                            foreach (tunedContentBand b in lst)
                                //List_Source.Add(b.pluginName + " (" + b.pluginDescription + ")");
                                List_Source.Add(new OMListItem(b.ToString(), "", format, b));
                        }
                        else
                        {
                            List_Source.Add(new OMListItem("No band available", "", format));
                        }
                    }
                    break;
                case "stationlist":
        ...
        public eLoadStatus initialize(IPluginHost host)
        {
            ...
            Radio_StationName.Format = textFormat.Bold;
            ...
            Radio_StationText.Format = textFormat.Normal;
            ...
            Radio_StationGenre.Format = textFormat.Normal;
            ...
            Radio_StationBitRate.Format = textFormat.Normal;
            ...
            Label_StationListHeader.Format = textFormat.Bold;
            ...
            Label_StationListSource.Format = textFormat.Normal;
        ...
        void ListView_List_OnClick(OMControl sender, int screen)
        {
            ...
            switch (PanelTag)
            {
                ...
                case "band":
                    {
                        if (SelectedItemTag == "No band available")
                            return;
                        for (int i = 0; i < theHost.ScreenCount; i++)
                            ClearStationInfo(i);
        ...
        private void UpdateStationInfo(int instance)
        {
            ...
            for (int i = 0; i < theHost.ScreenCount; i++)
            {
                ...
                if (((info.band == tunedContentBand.AM) | (info.band == tunedContentBand.FM)) && (info.currentStation.stationID.IndexOf(':') < (info.currentStation.stationID.Length-1)))
                {
